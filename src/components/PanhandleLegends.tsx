...
import { panhandleLegendsEpisodes, panhandleLegendsLinks, PANHANDLE_LEGENDS_DESCRIPTION, PANHANDLE_LEGENDS_PATH, PANHANDLE_LEGENDS_TITLE } from "../data/panhandle-legends";
import { absoluteUrl, collectionPageLd, jsonLdGraph } from "../lib/seo";
import { Seo } from "./Seo";
import "./PanhandleLegends.css";

export function PanhandleLegends() {
  ...
  const crumbs = [{ name: "United States", path: "/" }, { name: "Panhandle Legends", path: PANHANDLE_LEGENDS_PATH }];

  return (
    <div className="legends-page">
      <Seo
        title={PANHANDLE_LEGENDS_TITLE}
        description={PANHANDLE_LEGENDS_DESCRIPTION}
        policy="editorial"
        canonicalPath={PANHANDLE_LEGENDS_PATH}
        jsonLd={jsonLdGraph(
          collectionPageLd({ path: PANHANDLE_LEGENDS_PATH, name: "Panhandle Legends", description: PANHANDLE_LEGENDS_DESCRIPTION, crumbs }),
          {
            "@type": "ItemList",
      ...
      <section className="legends-hero" aria-labelledby="legends-title">
        <div className="legends-intro">
          <p className="kicker">Featured sponsor · Texas statewide</p>
          <h1 id="legends-title">Panhandle Legends</h1>
          ...
          <p className="legends-edition">Supporting The County Post’s Texas edition and all 254 Texas county editions.</p>
        </div>
      ...
      <section className="legends-connect" aria-labelledby="legends-connect-title">
        ...
        <div className="legends-link-grid">{panhandleLegendsLinks.map((link) => (
      ...
      <div className="legends-return"><Link to="/texas">← Explore Texas news</Link><Link to="/partners">Meet our partners</Link></div>
    </div>
  );
}
